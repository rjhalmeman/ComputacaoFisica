...
public class DAOSensor extends DAOGenerico<Sensor> {
    ...
    public Sensor listByNome(String nome) {
        try {
            return (Sensor) em.createQuery("SELECT e FROM Sensor e WHERE e.nomeSensor LIKE :nome").setParameter("nome", "%" + nome + "%").getSingleResult();
